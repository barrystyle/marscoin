...
// Copyright (c) 2014-2021 The Marscoin developers
...
#if defined(NDEBUG)
# error "Marscoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Marscoin Signed Message:\n";
...
bool GetTransaction(const uint256 &hash, CTransaction &txOut, uint256 &hashBlock, bool fAllowSlow)
{
    ...
    if (pindexSlow) {
        CBlock block;
        if (ReadBlockFromDisk(block, pindexSlow, true)) {
...
CAmount GetBlockValue(int nHeight, const CAmount& nFees)
{
    ...
    nSubsidy >>= halvings; // Marscoin: 395k blocks every Mars years (668.5991 sols)
...
void ThreadScriptCheck() {
    RenameThread("marscoin-scriptch");
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    if (!IsInitialBlockDownload() && !fWarned)
    {
        ...
        for (int i = 0; i < 100 && pindex != NULL; i++) {
            if (CBlockHeader::GetBaseVersion(pindex->nVersion) > CBlockHeader::GetBaseVersion(CBlockHeader::CURRENT_VERSION)) {
                ++nUpgraded;
            }
            pindex = pindex->pprev;
        }
...
bool static DisconnectTip(CValidationState &state) {
    ...
    CBlock block;
    if (!ReadBlockFromDisk(block, pindexDelete, true))
...
bool static ConnectTip(CValidationState &state, CBlockIndex *pindexNew, CBlock *pblock) {
    ...
    CBlock block;
    if (!pblock) {
        if (!ReadBlockFromDisk(block, pindexNew, true))
            return state.Abort("Failed to read block");
        pblock = &block;
    }
    // Apply the block atomically to the chain state.
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Marscoin: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet & regtest uses supermajority)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Marscoin: (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool CVerifyDB::VerifyDB(CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (!ReadBlockFromDisk(block, pindex, true))
            ...
            try {
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (ReadBlockFromDisk(block, it->second, true))
                        {
        ...
        const CInv &inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK)
            {
                ...
                if (send)
                {
                    ...
                    CBlock block;
                    if (!ReadBlockFromDisk(block, (*mi).second, true))
