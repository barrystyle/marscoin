    ...
    if (fGenerate && Params().MineBlocksOnDemand())
    {
        ...
        while (nHeight < nHeightEnd)
        {
            ...
            while (nMaxTries > 0 && miningHeader.nNonce < nInnerLoopCount && 
                   !CheckProofOfWork(miningHeader.GetHash(), pblock->nBits, Params().GetConsensus())) {
                ++miningHeader.nNonce;
                --nMaxTries;
            }
...
Value getauxblock(const JSONRPCRequest& request)
{
    if (request.fHelp
...
Value createauxblock(const JSONRPCRequest& request)
{
...
Value submitauxblock(const JSONRPCRequest& request)
{
