...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or marscoin.conf setting gen)\n"
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    if (fGenerate && Params().MineBlocksOnDemand())
    {
        ...
        const uint32_t nInnerLoopCount = 0x7fffffff;
        ...
        while (nHeight < nHeightEnd)
        {
            ...
            int nMaxTries = 1000000;  // Adjust as needed

            while (nMaxTries > 0 && miningHeader.nNonce < nInnerLoopCount && 
                   !CheckProofOfWork(miningHeader.GetHash(), pblock->nBits)) {
                ++miningHeader.nNonce;
                --nMaxTries;
            }

            if (nMaxTries == 0 || miningHeader.nNonce == nInnerLoopCount) {
                continue;
            }
...
static void AuxMiningCheck()
{
  ...
  LOCK(cs_main);
  if (chainActive.Height() + 1 < Params().MineBlocksOnDemand())
    throw std::runtime_error("mining auxblock method is not yet available");
}
static Value AuxMiningCreateBlock(const CScript& scriptPubKey)
{
    AuxMiningCheck();
...

    {
                ...
                && GetTime() - nStart > 60))
        {
            ...
            IncrementExtraNonce(&newBlock->block, const_cast<CBlockIndex*>(pindexPrev), nExtraNonce);
...
static bool AuxMiningSubmitBlock(const std::string& hashHex, const std::string& auxpowHex)
{
   ...
   const vector<unsigned char> vchAuxPow = ParseHex(auxpowHex);
...
Value submitauxblock(const Array& params, bool fHelp)
{
   ...
   return AuxMiningSubmitBlock(params[0].get_str(), params[1].get_str());
}